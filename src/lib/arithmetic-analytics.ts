...
export type ArithmeticAttempt={item:ArithmeticItem;answer:string;numericAnswer:number|null;correct:boolean;presentedAt:number;firstInputAt:number|null;submittedAt:number;firstInputMs:number;entryMs:number;responseMs:number;edits:number;backspaces:number;reason:ErrorReason;telemetryVersion?:2};
...
export function buildTrainingPlan(grade:ArithmeticGrade,sessions:ArithmeticSession[]):TrainingPlan{
 ...
 const metrics:SkillMetric[]=profile.skills.map(s=>{const a=bySkill.get(s.id)||[];const correct=a.filter(x=>x.correct).length;const errors=a.length-correct;const accuracy=a.length?correct/a.length:0;const think=a.map(x=>x.firstInputMs);const med=median(think);const medEntry=median(a.map(x=>x.entryMs??Math.max(0,x.responseMs-x.firstInputMs)));const speedRatio=a.length?med/s.targetMs:0;const editRate=a.length?a.filter(x=>x.edits>=2||x.backspaces>=2).length/a.length:0;let status:SkillStatus="unseen";if(a.length){if(errors>=2||(a.length>=6&&accuracy<profile.targetAccuracy-.1))status="needs_accuracy";else if(errors===1)status="monitor";else if(a.length<4)status="insufficient";else if(speedRatio>1.1)status="needs_fluency";else status="mastered"}const priority=a.length?Math.max(0,profile.targetAccuracy-accuracy)*5+Math.max(0,speedRatio-1)*.8+editRate*.5:0;const evidence=score100(a.length/8*100);const enough=a.length>=4;const accuracyScore=a.length?score100(accuracy/profile.targetAccuracy*100):null;const fluencyScore=enough&&med>0?score100(s.targetMs/med*100):null;const m=mean(think);const cv=m>0?stddev(think)/m:0;const stabilityScore=enough?score100(100-cv*80-editRate*30):null;const efficient=a.filter(x=>x.correct&&x.firstInputMs<=s.targetMs*1.1&&!(["strategy_missed","slow_recall","hesitation"] as ErrorReason[]).includes(x.reason)).length;const strategyScore=enough?score100(efficient/a.length*100):null;return{skillId:s.id,attempts:a.length,correct,accuracy,medianMs:med,medianEntryMs:medEntry,speedRatio,editRate,priority,status,evidence,accuracyScore,fluencyScore,stabilityScore,strategyScore}}).sort((a,b)=>b.priority-a.priority);
