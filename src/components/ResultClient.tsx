...
const UI={
  zh:{noResult:"暂无考试结果",backList:"返回考试列表",result:"本次成绩",time:"完成时间",correct:"正确",wrong:"错误",blank:"空题",difficulty:"按难度",points:"分题",concepts:"知识点表现",review:"逐题复盘",again:"再考一次",list:"考试列表"},
  en:{noResult:"No exam result yet",backList:"Back to exams",result:"Your result",time:"Time used",correct:"Correct",wrong:"Wrong",blank:"Blank",difficulty:"By difficulty",points:"-point",concepts:"Performance by skill",review:"Review answers",again:"Try again",list:"Exam list"},
} as const;
...
export default function ResultClient(){
  ...
  return <div className="report-shell">
    ...
    <div className="report-actions"><Link className="primary-button" href="/review">{ui.review}</Link><Link className="secondary-button" href={`/exam/${examId}`}>{ui.again}</Link><Link className="secondary-button" href="/">{ui.list}</Link></div>
  </div>;
}
