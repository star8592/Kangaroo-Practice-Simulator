...
type Attempt={grade:GradeResult;elapsedSeconds:number;submittedAt:number};
export default function ResultClient(){
  ...
  if(!a)return <div className="center-card"><h2>暂无考试结果</h2><Link className="primary-button" href="/exam/level-a">开始一场考试</Link></div>;
  ...
  return <div className="report-shell">
    <section className="score-hero"><div><span className="eyebrow">LEVEL A · RESULT</span><h1>本次成绩</h1><p>完成时间 {mins}:{String(secs).padStart(2,"0")}</p></div><div className="score-ring"><strong>{g.score}</strong><span>/ {g.maxScore}</span><small>{pct}%</small></div></section>
    ...
    <div className="report-actions"><Link className="primary-button" href="/review">逐题复盘</Link><Link className="secondary-button" href="/exam/level-a">再考一次</Link></div>
  </div>;
}
