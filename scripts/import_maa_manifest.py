...
ROOT=Path(__file__).resolve().parents[1]; PUB=ROOT/'public/local-assets/maa-amc'; PRIVATE_ASSETS=ROOT/'private/source-archive/maa-amc'; EXAMS=ROOT/'private/exams'; DPI=170; SCALE=DPI/72
FORMATS={
...
def build(entry,root):
 source_id=entry.get('sourceRegistryId')
 if not source_id: raise RuntimeError(f"{entry.get('id')}: sourceRegistryId is required")
 ...
 fmt=FORMATS[entry['formatId']]; problems=root/entry['problems']; ans=answers(entry,fmt,root)
 public_allowed=bool(policy['publicQuestionDisplay'])
 asset_root=PUB if public_allowed else PRIVATE_ASSETS
 out=asset_root/str(entry['year'])/entry['id']; crop(problems,fmt['count'],out); rz,re=rules(entry,fmt);choices=[{'key':k,'label':k} for k in 'ABCDE'];qs=[]
 for q in range(1,fmt['count']+1):
  url=f"/local-assets/maa-amc/{entry['year']}/{entry['id']}/q{q:02}.png" if public_allowed else None
  qs.append(dict(id=f"{entry['id']}-q{q:02}",year=entry['year'],level=entry.get('label',entry['formatId']),grades=entry['grades'],language=entry.get('language','en'),questionNo=q,points=fmt['points'],answerMode=fmt['mode'],concept='official_original',stem=f"{entry.get('label',entry['formatId'])} · Question {q}",choices=choices if fmt['mode']=='choice' else [],answer=ans[q],solution=f"Correct answer: {ans[q]}",sourceFile=str(problems),assetUrl=url,studentAssetUrl=url,verified=True,examReady=public_allowed,sourceRegistryId=source_id,rightsClass=policy['rightsClass'],publicQuestionDisplay=public_allowed))
 profile=dict(id=entry['id'],name=entry.get('name',entry['id']),nameZh=entry.get('nameZh',entry.get('name',entry['id'])),nameEn=entry.get('nameEn',entry.get('name',entry['id'])),grades=entry['grades'],gradesZh=entry.get('gradesZh',entry['grades']),gradesEn=entry.get('gradesEn',entry['grades']),durationSeconds=fmt['minutes']*60,questionCount=fmt['count'],initialScore=0,maxScore=fmt['max'],wrongPenaltyMode='fixed',wrongPenaltyValue=0,blankScoreValue=fmt['blank'],country='MAA AMC',year=entry['year'],language=entry.get('language','en'),sourceLabel=entry.get('sourceLabel','MAA AMC authorized/local source'),studentReady=public_allowed,competitionId='maa-amc',formatId=entry['formatId'],paperType=entry.get('paperType','past'),gradeBand=fmt['band'],timingMode=entry.get('timingMode','official'),formatLabelZh=entry.get('formatLabelZh',entry.get('label',entry['formatId'])),formatLabelEn=entry.get('formatLabelEn',entry.get('label',entry['formatId'])),rulesSummaryZh=rz,rulesSummaryEn=re,sourceRegistryId=source_id,rightsPolicy=policy,**({'timingSections':fmt['sections']} if fmt.get('sections') else {}))
 EXAMS.mkdir(parents=True,exist_ok=True);(EXAMS/f"{entry['id']}.json").write_text(json.dumps({'profile':profile,'questions':qs},ensure_ascii=False,indent=2)+'\n');print('IMPORTED',entry['id'],fmt['count'],'PUBLIC' if public_allowed else 'INTERNAL_ONLY')
def main():
