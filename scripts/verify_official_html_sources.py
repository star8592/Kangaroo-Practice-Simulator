...
def main():
 ...
 for j in q.get('jobs',[]):
  ...
  if not textual_choices: rejected.append((j,'choices_not_structured_text'));continue
  if not all(v in txt for v in actual): rejected.append((j,'choice_not_found_verbatim'));continue
  rows.append({'examId':j['examId'],'questionNo':j['questionNo'],'sourceLanguage':j.get('sourceLanguage','en'),'sourceText':stem,'choices':choices,'sourceFile':str(p),'sourceSha256':hashlib.sha256(raw).hexdigest(),'extractionMethod':'official_html','verificationStatus':'SOURCE_VERIFIED','verificationMethod':'deterministic_verbatim_html','assetUrl':j.get('assetUrl')})
