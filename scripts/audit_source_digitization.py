...
def main():
 ...
 for j in data.get('jobs',[]):
  ...
  status='SOURCE_VERIFIED' if not issues and origin in {'html','manual','official_pdf_manual_repair'} else ('NEEDS_SOURCE_REVIEW' if text else 'RAW')
  rows.append({'examId':j.get('examId'),'questionNo':j.get('questionNo'),'origin':origin,'status':status,'issues':issues,'assetUrl':asset})
