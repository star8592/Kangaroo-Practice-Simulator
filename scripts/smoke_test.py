...
ROOT=Path(__file__).resolve().parents[1];USERS=ROOT/'private/users/users.json';ATT=ROOT/'private/users/exam-attempts.jsonl';SESS=ROOT/'private/users/exam-sessions.json';PREA='au-amc-pre-a-sample-1';AMC='au-amc-2021-middle-primary';MAA8_EN='maa-amc8-2023-sample';MAA8_ARCHIVE='maa-amc8-2000-user-owned';MAA8_2024='maa-amc8-2024-user-owned';MAA8_SMART='smart-maa-amc8--smokeverify';MAA10_EN='maa-amc10-2022-a-sample';MAA_PRACTICE_EN='maa-amc10-practice-algebra'
...
def main():
 ...
 try:
  ...
  code,_=req(client,base+f'/api/exams/{MAA8_EN}');assert code==404,code
  code,m8a=req(client,base+f'/api/exams/{MAA8_ARCHIVE}');assert code==200 and len(m8a['questions'])==25 and m8a['profile']['competitionId']=='maa-amc' and m8a['profile']['formatId']=='maa-amc8' and m8a['profile']['paperType']=='past' and m8a['profile']['year']==2000 and m8a['profile']['language']=='zh/en' and all('answer' not in q for q in m8a['questions']) and all(q.get('language')=='zh/en' for q in m8a['questions']) and 'archive-amc8/2000/q01.png' in (m8a['questions'][0].get('assetUrl') or '')
  code,m824=req(client,base+f'/api/exams/{MAA8_2024}');assert code==200 and len(m824['questions'])==25 and m824['profile']['competitionId']=='maa-amc' and m824['profile']['formatId']=='maa-amc8' and m824['profile']['paperType']=='past' and m824['profile']['language']=='zh/en' and all('answer' not in q for q in m824['questions']) and all(q.get('language')=='zh/en' for q in m824['questions']) and '2024/AMC_8_bilingual/q01.png' in (m824['questions'][0].get('assetUrl') or '')
  code,m8s=req(client,base+f'/api/exams/{MAA8_SMART}');assert code==200 and len(m8s['questions'])==25 and m8s['profile']['competitionId']=='maa-amc' and m8s['profile']['formatId']=='maa-amc8' and m8s['profile']['paperType']=='smart' and m8s['profile']['durationSeconds']==2400 and m8s['profile']['maxScore']==25 and all('answer' not in q for q in m8s['questions']) and all(q.get('language')=='zh/en' for q in m8s['questions'])
  code,_=req(client,base+f'/api/exams/{MAA10_EN}');assert code==404,code
  code,_=req(client,base+f'/api/exams/{MAA_PRACTICE_EN}');assert code==404,code
  code,_=req(client,base+'/api/exam-sessions','POST',{'examId':MAA10_EN});assert code==404,code
  ...
  code,start=req(client,base+'/api/exam-sessions','POST',{'examId':MAA8_2024});assert code==200;code,maa8_blank=req(client,base+'/api/grade','POST',{'examId':MAA8_2024,'sessionId':start['session']['id'],'answers':{},'lang':'zh','events':[]});assert code==200 and maa8_blank['score']==0
  code,ana=req(client,base+'/api/student/analytics');assert code==200 and ana['overview']['examAttempts']==3 and ana['overview']['totalQuestions']==75 and ana['practice']['attempts']==0 and ana['practice']['questions']==0 and ana['readiness'] is not None and ana['dataConfidence']>0
  ...
  print(json.dumps({'auth':'PASS','pre_a_questions':25,'pre_a_distribution':{'3':10,'4':10,'5':2,'6':2,'8':1},'pre_a_untimed':True,'pre_a_full_score':100,'pre_a_blank_score':0,'amc_middle_primary_minutes':60,'amc_middle_primary_max':135,'amc_middle_primary_bilingual':True,'answer_leak':False,'student_admin_access':403,'maa_amc8_archive_2000':25,'maa_amc8_2024':25,'maa_amc8_smart':25,'english_only_student_blocked':True,'maa_practice_separate':True,'analytics_attempts':3,'analytics_questions':75,'login_rate_limit':True,'session_revocation':True,'arithmetic_next_session':True},ensure_ascii=False))
 finally:cleanup(uid)
